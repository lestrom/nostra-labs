...
import twitterAuthRouter from "./routes/auth-twitter.js";
...
import { NgrokService } from "./services/ngrok.service.js";
import { ContractMonitorService } from "./services/contractMonitor.service.js";
...
app.listen(port, async () => {
  try {
    ...
    try {
      ...
      console.log(
        "[WARN] Unable to use twitter. Functionality will be disabled",
        err
      );
    }

    const contractMonitorService = ContractMonitorService.getInstance(
      telegramService.elizaService
    );
    // Initialize contract monitor service
    contractMonitorService.start();
  } catch (e) {
